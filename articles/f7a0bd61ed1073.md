...
おうちで24時間稼働しているサーバを停電、雷から守るため
[CP750PFCLCD JP](https://www.cyberpower.com/jp/ja/product/sku/cp750pfclcd_jp)を購入しました。
...
flowchart LR
  ...
  node_2["Proxmox\n[PowerPanel]"]
  node_3["Raspberry Pi"]
  node_1 --"停電を検知してバッテリーに切り替えたけど、<br>そろそろ残量がなくなるぞ！"--> node_2
...
↓
Raspberry Piに`shutdown`コマンドをSSHで送り、その後にProxmox自身もシャットダウン

# 🛠 準備をしよう

### Powerpanelのインストール

ProxmoxのホストOSに[PowerPanel for Linux 64bit (deb)](https://www.cyberpower.com/jp/ja/product/sku/powerpanel_for_linux#overview)をダウンロードします。
...
不要な項目を無効化、
バッテリー残量が一定の基準に低下したらシャットダウンするように編集します。
...
スクリプトの冒頭にラズパイへのコマンドを追記します。
:::message
事前に公開鍵認証の設定、ラズパイ上でパスワードなしでsudoコマンドを使えるように設定してください。
:::

```diff sh:/etc/shutdown.sh
#!/bin/sh

+ ssh user@host 'sudo shutdown -h now'  # ラズパイ

shutdown -h now
```

これでラズパイをシャットダウンさせてからProxmox自身もシャットダウンすることができます。